...
class achyb : public ModulePass {
private:
  virtual bool runOnModule(Module &) override;
  bool achybPass(Module &);

  // Yang:
  void achyb_process(Module &module);

  std::unordered_map<Function*, std::list<CallInst*>> caller_map;
...
public:
  ...
  virtual StringRef getPassName() const override { return "achyb"; }
...
private:
  ...
  // Yang l
  std::unordered_map<Function*, CallInst*> critical_evidences;
...
private:
  ...
  inline bool is_skip_function(const std::string &str) const {
    return skip_funcs->exists_ignore_dot_number(str) ||
           kernel_api->exists_ignore_dot_number(str); // || str == "llvm.dbg.value"
  };
